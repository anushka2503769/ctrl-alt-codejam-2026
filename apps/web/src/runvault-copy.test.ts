...
describe("RunVault outcome language", () => {
  it("defines copy for every persisted enum value", () => {
    ...
    const reasons: RunVaultReason[] = ["verified_safe", "protected_path", "change_limit_exceeded", "dependency_change", "verification_failed", "run_failed", "cancelled", "timed_out", "unsafe_file", "unsafe_link", "trusted_workspace_changed"];
    const resolutions: RunVaultResolution[] = ["policy", "human_approved", "human_discarded"];
    const findings: RunVaultFindingCode[] = ["execution_cancelled", "execution_timed_out", "execution_failed", "verification_failed", "trusted_workspace_changed", "unsafe_link", "protected_path", "dependency_change", "unsafe_file", "change_limit_exceeded", "deletion_limit_exceeded"];
